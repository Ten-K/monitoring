  ...
  <div id="baseTableCom">
    ...
    <div class="tableDiv" :style="tableStyle">
      <el-table
        ...
        <el-table-column type="expand" v-if="isExpand" :class-name="hasRowHeaderExpand ? 'hasRowHeaderExpandPaddingTop' : ''">
        ...
        <el-table-column
          ...
          <template v-slot="scope">
            <div>
              <!-- 行的头部 -->
              <div class="rowHeader" v-if="hasRowHeaderExpand && index === 0" :style="{'left': rowHeaderStyleLeft}">
                <slot name="rowHeader" :scope="scope"></slot>
              </div>

              <!-- 文本 -->
              <p v-if="item.type === 'word' || item.columnOperable === 'none'" slot="reference" v-text="getCellWord(scope, item)" class="word"></p>

              <!-- 特殊元素 -->
              <slot v-else-if="item.isSpecial" :name="item.prop" :scope="scope"></slot>
              
              <!-- 图片 -->
              <div v-else-if="item.type === 'image'">
                <img 
                  :src="scope.row[item.prop]" 
                  @click="zoomImg(scope.row[item.prop])" 
                  style="vertical-align: middle;height: 29px;width: 35px;cursor: pointer;" />
              </div>
              ...
              <el-popover
                ...
                <div 
                  ...
                  <el-button 
                    v-if="formLabelsArr[scope.$index][index]['comData']['type'] === 'detail'" 
                    type="primary" 
                    size="mini" 
                  ...
                  <div v-else>
                    <el-button 
                      type="primary" 
                      size="mini" 
                      @click="saveCellChange(formLabelsArr[scope.$index][index])">
                      保存
                    </el-button>
                  </div>
                </div>
                <p slot="reference" v-text="getCellWord(scope, item)" class="word"></p>
              </el-popover>
...
export default {
  ...
  watch: {
    ...
    tableData: {
      handler(newVal){
        ...
        if(this.listData.length === 0 && data.pageNo !== 0){
  ...
  methods: {
      ...
      for(let i=0; i < this.listData.length; ++i){
        ...
        this.tableHeader.forEach((itemInTH, indexInTH) => {
          ...
          if(!excludeFormItemName.includes(itemInTH.type)){
            ...
            //处理表单元素的fns属性
            obj.formLabels[0].fns = this.handleFn(itemInTH, this.listData[i], i, indexInTH)
          }
          arr.push(obj);
        })
    ...
    handleFn(formItemConfig, data, rowIndex, columnIndex){
      if(!formItemConfig.fns)
        return null;
      let fns = {};
      //绑定传入的参数
      Object.keys(formItemConfig.fns).forEach(itemInF => {
        fns[itemInF] = (value) => {
          formItemConfig.fns[itemInF].call(this, value, {
            rowIndex,
            columnIndex,
            ...formItemConfig
          }, data);
    ...
    handleCellColor: function(data){
      ...
      if(specialColumn.includes(data.column.type))
        return;
      let index = data.columnIndex - (this.canSelect ? 1 : 0) - (this.isExpand ? 1 : 0);
      if(this.$utils.funcData.isEmpty(this.tableHeader[index]) 
        || this.$utils.funcData.isEmpty(this.tableHeader[index]['colorAndVal']))
        return;
      let val = Object.values(data.row)[index];
      let resultColor = this.tableHeader[index]['colorAndVal'][val] ? this.tableHeader[index]['colorAndVal'][val] : null;
    ...
    handleSizeChange(pageSize){
      ...
      if(this.showPage){
        ...
        sessionStorage.setItem(`${this.comData.id}Page`, JSON.stringify(pageData));
      }
    ...
    sortList(item, param1, param2){
      if(!this.$utils.funcData.isNaN(Number(param1[item.prop]))){
        ...
        let arr1 = String(param1).split('').map(e => {
          return String(e.charCodeAt());
        });
        let arr2 = String(param1).split('').map(e => {
          return String(e.charCodeAt());
        });
