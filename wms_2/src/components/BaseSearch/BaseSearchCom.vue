  ...
  <div id="baseSearchCom">
    ...
    <div class="form">
      <div
        ...
        <div :style="{width: `calc(100% - ${+ labelWidth + 15}px)`, display: 'inline-block'}">
          <!-- 
            输入框
            {
              type: input,
          ...
          <template v-else-if="item.type === 'datePicker'">
            <el-date-picker
              v-model="formData[dateParamName(item.prop, 'from')]"
            ...
            <el-date-picker
              v-model="formData[dateParamName(item.prop, 'to')]"
              :type="item.itemType ? item.itemType : 'date'"
              :size="formSize"
              :clearable="false"
      ...
      <div class="formItem" style="margin-left: 5px;">
        <el-button class="btn" :size="formSize" type="primary" @click="searchData">查询</el-button>
        <el-button class="btn" :size="formSize" @click="clearFormData" v-if="showResetBtn">重置</el-button>
      </div>
    </div>
  ...
  methods: {
    ...
    searchData(){
      ...
      for(let i=0; i < this.formLabel.length; i++){
        if(this.formLabel[i].type === 'datePicker' 
          && !this.formData[this.dateParamName(this.formLabel[i].prop, 'from')]
          && this.formData[this.dateParamName(this.formLabel[i].prop, 'to')]){
        ...
        if(this.formLabel[i].type === 'datePicker' 
          && this.formData[this.dateParamName(this.formLabel[i].prop, 'from')]
          && this.formData[this.dateParamName(this.formLabel[i].prop, 'from')] > this.formData[this.dateParamName(this.formLabel[i].prop, 'to')]){
